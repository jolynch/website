...
with the basics. In this two part series I will cover the basics of
...
stream processing from a ordered log like
[Flink](https://ci.apache.org/projects/flink/flink-docs-release-1.7/) and even
background processing tasks such as sending emails or resizing images from a
[SQS](https://aws.amazon.com/sqs/) queue worker.

Many performance bugs boil down to something along the lines of:

1. \[`task`\] starts taking twice as long
2. The queues containing \[`task`\]s grows without bound
3. Everything dies
...
It turns out that the single queue strategy is, *by far*, the better option,
leading to superior [latency](https://en.wikipedia.org/wiki/Latency_(engineering)),
and therefore also [throughput](https://en.wikipedia.org/wiki/Throughput) for a
given number or processors. This may not always be a good thing if we, for
example, queue short tasks behind large ones, but in general having more
throughput is good.
...
From [M. Harchol-Balter](#harchol_balter) (Section V: Server Farms and
Networks, pg ~263) we can work out the expected latency of these two systems:
...
a multiple of the request rate (requests are queueing faster than they are
serviced).
...
a 10 server farm with average response latencies of `0.4ms` per request and a
request rate of `18,000` requests per second we see that the single queue does
about **`3x`** better than multiple queues on average across the latency
distribution:
...
**Summary**: Please, for the love of all that is good, keep work in a single queue
if you can.
...
3. **`Best of Two Adjacent`**: This is a variant of the classic "choice of two"
   ...
   checkouts this one picks a random checkout and picks between that queue and
...
(about 10% worse across the board).
...
_right now_. If you can't do that, then try to have a good load balancer.
...
of this (although as we will see in part 2, estimating queued work is not
exactly the same as the shortest queue).
...
1. Use a smaller number of queues when you are worker bound that dispatch to
   ...
   throughput and decrease mean latency.
2. If you must balance between queues, choose something better than random
   load balancing. Ideally join queues with the shortest remaining work.
